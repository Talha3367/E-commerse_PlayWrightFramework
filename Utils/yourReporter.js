...
export default class MyReporter {
  ...
  async onEnd(result) {
    ...
    if (sharePointData.sendReportToTeams) {
      let teamsMessage = `**Playwright Operation: Tests Completed `;
      teamsMessage += result.status === 'passed' ? '(All Succeeded)**' : '(With Failures)**';
      teamsMessage += await this.createMessageForTeams();
